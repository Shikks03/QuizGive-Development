...
window.parseQuizfetch = function parseQuizfetch(htmlString) {
  ...
  // strip noisy "[S1-SUMMATIVE] " bracket-prefix if it duplicates the title.
  // keep it for context; just normalise whitespace.
  ...
  const questions = cards.map((card, i) => {
    ...
    prompt = prompt.replace(/^\s*\d+\.\s*/, ''); // strip leading "1. "
    // tidy smart quotes that quizfetch emits
    ...
    // Keep only real choices; preserve the correct flag mapping.

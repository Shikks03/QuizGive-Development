/* global React */
// QuizGive — persistent state hook backed by localStorage.
...
const QG_DEFAULTS = {
  ...
  quizzes: {},                 // { [quizId]: Quiz }
  sessions: {},                // { [quizId]: Session }   in-progress + last finished
  results: {},                 // { [quizId]: lastResultSummary }
  bookmarks: [],               // [quizId, …]
  questionBookmarks: {},       // { [quizId]: number[] }   (question indices)
  recentQuizId: null,
  ranOnboarding: false,
};
...
window.useQGStore = function useQGStore() {
...
window.QGHelpers = {
  ...
  // Build a fresh session for a quiz given config
  makeSession(quiz, config) {
    const n = quiz.questions.length;
    const questionOrder = config.randomizeQuestions
      ? window.QGHelpers.shuffle(Array.from({ length: n }, (_, i) => i))
      : Array.from({ length: n }, (_, i) => i);
    const choiceOrders = quiz.questions.map((q) => {
      const indices = Array.from({ length: q.choices.length }, (_, i) => i);
      return config.randomizeChoices ? window.QGHelpers.shuffle(indices) : indices;
    });
    return {
      quizId: quiz.id,
      startedAt: Date.now(),
      finishedAt: null,
      layout: config.layout || 'one',            // 'one' | 'all'
      feedback: config.feedback || 'submit',     // 'instant' | 'submit'
      randomizeQuestions: !!config.randomizeQuestions,
      randomizeChoices: !!config.randomizeChoices,
      questionOrder,
      choiceOrders,
      answers: Array(n).fill(null),              // index INTO original choices, null if unanswered
      currentIdx: 0,                             // index into questionOrder
      submitted: false,
    };
  },

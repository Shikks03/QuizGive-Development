/* global React, QGIcon, QGHelpers */
// Quiz-taking + Results screens.

const { Star, StarFill, Check, X, ArrowRight, ArrowLeft, ChevLeft, ChevRight, Refresh, Award, Download, Sparkles, Plus, Eye, EyeOff, Clock } = window.QGIcon;

// ── Quiz Screen ────────────────────────────────────────────────────
window.QGQuizScreen = function QGQuizScreen({ state, actions, navigate, quizId }) {
  const quiz = state.quizzes[quizId];
  const existingSession = state.sessions[quizId];

  // If quiz finished previously and there's a result → show results instead of starting over.
  // User can retake from there.
  const result = state.results[quizId];
  if (result && (!existingSession || existingSession.submitted)) {
    return <window.QGResultsScreen state={state} actions={actions} navigate={navigate} quizId={quizId} />;
  }

  // If we landed here without a session, build a default one (sensible defaults).
  React.useEffect(() => {
    if (!quiz) return;
    if (!existingSession) {
...
function OneAtATime({ state, actions, navigate, quiz, session }) {
  ...
  const qOrig = session.questionOrder[idx];   // original index in quiz.questions
  const q = quiz.questions[qOrig];
  const order = session.choiceOrders[qOrig];  // shuffled choice indices
  const answered = session.answers[qOrig];    // null or original choice index
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <div className="qg-dotgrid">
              {session.questionOrder.map((qOrig2, i) => {
                ...
                if (ans != null) stateClass = 'ok'; // answered (we don't reveal correctness mid-quiz)
                return (
      ...
      <div className="qg-scroll">
        <div className="qg-content wide">
          ...
          {visibleQOrigs.map((qOrig, displayN) => {
            ...
            // displayN refers to position in original session order; we need the user-facing number
...
// ── Results Screen ─────────────────────────────────────────────────
window.QGResultsScreen = function QGResultsScreen({ state, actions, navigate, quizId }) {
  const quiz = state.quizzes[quizId];
  const session = state.sessions[quizId];
  const result = state.results[quizId];
  const [filter, setFilter] = React.useState('all');   // 'all' | 'wrong' | 'bookmarked'
  const bookmarks = state.questionBookmarks[quiz?.id] || [];
    ...
    <>
      <window.QGTopbar
        ...
        right={<>
          ...
          <button className="qg-btn sm" onClick={() => window.QGExport?.downloadResults(quiz, session, result)}>

...
// ── Upload ───────────────────────────────────────────────────────
window.QGUploadScreen = function QGUploadScreen({ state, actions, navigate, sample }) {
...
// ── Preview & configure ──────────────────────────────────────────
window.QGPreviewScreen = function QGPreviewScreen({ state, actions, navigate, quizId }) {
...
function CheckRow({ checked, onToggle, icon, label }) {
  return (
    <div className={`qg-check-row ${checked ? 'on' : ''}`} onClick={onToggle}>
      <span className="box">{checked && <window.QGIcon.Check size={12} />}</span>
...
// ── Ready / decision ─────────────────────────────────────────────
window.QGReadyScreen = function QGReadyScreen({ state, actions, navigate, quizId }) {
  const quiz = state.quizzes[quizId];
        ...
        <div className="qg-card flat" style={{ marginTop: 16, padding: '10px 14px', background: 'var(--surface-2)', borderStyle: 'dashed' }}>
          <div className="qg-row between" style={{ flexWrap: 'wrap', gap: 8 }}>
            <div className="qg-row qg-muted-2" style={{ fontSize: 13 }}>
              <window.QGIcon.Download size={14} />
              Also: <a onClick={(e) => { e.preventDefault(); window.QGExport?.downloadJSON(quiz); }} href="#">download JSON</a>
              · <a onClick={(e) => { e.preventDefault(); window.QGExport?.downloadHTML(quiz); }} href="#">download HTML</a>
            </div>
